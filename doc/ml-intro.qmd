...
are selected to start as easy as possible, but can of course not give
...
$w_i$. The weighted inputs are then combined with the help of a
summation function and passed on to the output $y$ via a transfer
function.

The transfer function can have different structure, where a sigmoidal
...
The sigmoidal function can look very different, depending on weights
...
```
--->
![ully connected feed-forward network with three layers, for example and 3 neurons in the input  layer, 5 in the hidden layer and 2 in the output.  Deep neural networks have more than one hidden layer.](img/diagrammer-ann.png){#fig-forward-network width=60%}
...
```{r}
#| label: fig-toydata
#| fig-cap: "Toy data set and fitted neural networks with 1 and 5 hidden neurons."
...
Now we can fit a neural network:
...
then be used in `predict` where the variable namesof the new data mus exactly match 
...
2. Now try to fit the whole data set with all four species. Find good
   ...
   detemination and a plausible figures.
...
inputs for the neural network, while phytoplankton biomass (XBM) in
the epilimnion was used as output. The DATE variable will be used to
...
In effect, the `task`-object modifies itself without the need ot an assignment 
with `<-`or `=`.
...
if you want. The random forest learner is more robust and works with the defauolt 
settings.
...
hihghly CPU intensive computations, but the data set is small,
...
The values of RMSE and  R squared can be misleading, because it is the value for the training data. 
To get it for the test data, w first do a prediction for the test data subset and then compute the scores.
...
forest models, it cannot be applied for all learners. A list of available
