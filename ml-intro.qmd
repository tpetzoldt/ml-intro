...
In our research we are working with additional methods like Gradient Boosting 
[@Friedman2001gbm] and Keras [@Chollet2015keras], but I had not time yet to document 
didactical examples here. Additional references are found in the "Further Reading" 
section.

We start with a technical example and then apply the methodology to
practical data sets.
...
and then passed on to the output $y$ via a transfer (or activation) function.

The activation function can have different structure, where in the classical case,
...
#| label: fig-sigmoid
#| fig-cap: "Flexibility of a sigmoidal activation function. A single neuron can 
...
network frameworks like [Keras](https://keras.io/) prefer other activation functions, 
...
several advantages. It needs less computation, can be simplified to "zero" 
...
While ReLU is the modern standard, even newer functions like Swish 
(`x * sigmoid(x)`) are becoming popular. Swish retains the benefits of ReLU 
(fast computation and non-saturation) but adds a smooth 'dip' at zero. 
This smoothness often helps models converge more reliably on complex, 
curvy data. For details see [Swish_function](https://en.wikipedia.org/wiki/Swish_function) in Wikipedia.
...
wiggles. This is ok for **pattern recognistion**. However, for **forecasting**, 
one must strictly avoid "non-causal" interpolation  (like splines or standard 
linear interpolation) on both target and predictor variables.
These methods "leak" information from the future into the past, leading to
artificially optimistic results. For such cases, causal methods or 
forward-filling are preferred. 
...
* Hands-On Machine Learning with R [@Boehmke2019]
* An introduction to statistical learning with applications in R [@James2021]
* Deep learning with R [@Chollet2022]
* Feature Engineering A-Z [@Hvitfeldt2024]
* TensorFlow for R [@tensorflow_tutorials]
